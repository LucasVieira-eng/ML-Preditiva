...
def load_cmaps_data(file_path, window_size=120, rul_clip=200, min_sequence_length=40):
    """
...
def build_cnn_model(input_shape, l2_strength=1e-3):
    return models.Sequential([
        ...
        layers.Dense(1)  # No activation - unbounded predictions in log-space
    ])
...
def train_cnn_model(model, X_train, Y_train, model_path, epochs=50, batch_size=64, 
                    validation_split=0.2, high_rul_scale=1.0):
    """
    Train CNN model with optional sample weighting for high RUL values.
    ...
    Args:
        ...
        use_quantile_loss: If True, use QuantileLoss (more robust to outliers) instead of Huber
    """
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(os.path.dirname(__file__), '../../models/CNN_modelV2.keras')
    window_size = 80
    ...
    train_cnn_model(model, X_train, Y_train, model_path, high_rul_scale=high_rul_scale)
